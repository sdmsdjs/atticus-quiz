...
const tokenToIndex = (token: string): number | null => {
    ...
    const letterIndex = labels.indexOf(normalized);
    return letterIndex >= 0 ? letterIndex : null;
};

const getCorrectIndexes = (question: Question): Set<number> => {
...
const getFillAnswer = (question: Question): string => {
    const rawAnswer = question.correctAnswer.trim();
    const answerIndex = tokenToIndex(rawAnswer);

    if (answerIndex !== null && question.options[answerIndex]) {
        return question.options[answerIndex];
    }

    if (rawAnswer && rawAnswer !== '1') {
        return rawAnswer;
    }
...
const toSlidePart = (question: Question): number => {
    if (question.questionType === QuestionType.Checkbox) return 2;
    if (question.questionType === QuestionType.FillInTheBlank) return 3;
    return 1;
};

export const buildHtmlSlides = (questions: Question[]): HtmlQuizSlide[] => {
    const counters = new Map<number, number>();

    return questions.map(question => {
        const part = toSlidePart(question);
        const count = (counters.get(part) || 0) + 1;
        counters.set(part, count);

        const correctIndexes = getCorrectIndexes(question);
        const time = Number(question.timeInSeconds) || 60;
        ...
        if (question.questionType === QuestionType.Checkbox) {
            return {
                ...
                options: question.options.map((option, index) => ({
                    ...
                    text: option,
                    is_correct: correctIndexes.has(index),
                })),
                correct_answer: '',
                image: question.imageLink || undefined,
                time,
            };
        }
        ...
        if (question.questionType === QuestionType.FillInTheBlank) {
            return {
                ...
                explanation: question.answerExplanation,
                options: [],
                correct_answer: getFillAnswer(question),
                image: question.imageLink || undefined,
                time,
            };
        }
...
const toScriptJson = (slides: HtmlQuizSlide[]): string =>
...
export const exportToHtml = (questions: Question[], fileName = `${makeExportBaseName(questions)}.html`) => {
    const baseName = fileName.replace(/\.html?$/i, '');
    downloadTextFile(buildHtmlDocument(questions, baseName), fileName, 'text/html;charset=utf-8');
};

export const createHtmlPreviewUrl = (questions: Question[], title = makeExportBaseName(questions)): string => {
    const blob = new Blob([buildHtmlDocument(questions, title)], { type: 'text/html;charset=utf-8' });
